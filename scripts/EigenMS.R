...
# yuliya.k@gmail.com
#
# Version has been split into 2, eig_norm1 finds significant bias trends,
...
# if the number of automatically identified bias trends are close to that number we suggest using that number.
# 
# eig_norm2 normalizes the data, then rescales to avoid overfitting. 
#
# EigenMS estimates and preserves fixed effects. Random effects may be attempted in the future. 
# Contributions are welcome. 
#
# Code for rescaling Present in the initial paper has been removed from the R version. 
# Metabolomics in particular has a lot of variation that still remains that we concluded 
# there is not need for rescaling. Code available upon request: yuliya.k@gmail.com
#
# HOWTO RUN: 
# source('.../EigenMS.R')
# dd = # read in the data
...
eig_norm1 = function(m, treatment, prot.info, write_to_file=''){
# Identify significant eigentrends, allow the user to adjust the number if desired
# before normalizing
# 
# Input:
...
#   ncompl - number of complete peptides (?)
  ...
  ### QUESTION: do we really need to filter, or can we do try/catch around 
  ### model matrix creatiton, thus for a peptide that cannot get estimated, 
  ### we do nothing in the catch, else we estimate everything ...
  ...
  # 'nested' groups for single layer group leave as it is 
  grpFactors = treatment # temporary var, no real reason...
  ...
  if(nGrpFactors > 1) { # got nested factors
    ...
    # assume grp is a factor, so do nothing here...
    # grp = data.frame(lapply(grpFactors, as.factor)) # for DanteR, as.factor(grpFactors)
    # grp = grp[,1]
    # grp = as.factor(grpFactors)
    grp = treatment
  }
...
print(grp)
  ...
  # yuliya: 'present' are names of the peptide (pepID) and 'pres' are abundances...
...
print('Selecting complete peptides')
  ...
  # yuliya: let's write out a file of complete peptides if file name is passed in
  ...
  # if have only 1 group, we do not need to preserve group differernces, eveything is the same group, ex: QC samples
  ...
  if(n.u.treatment > 1) { 
    ...
    attach(TREAT)
    # mod.c <- model.matrix(lm.fm$lm.formula, data=treatment, eval(parse(text=lm.fm$lm.params)))  
  ...
  # SVD can fail sometimes, need to use try/catch?
  # #   Caution: have not tested this much.
...
#  plot.eigentrends(my.svd, "Residual Data")
  ...
  retval <- list(m=m, treatment=treatment, my.svd=my.svd,
            ...
            h.c=h.c, present=present, ptot.info=prot.info,
            complete=complete, toplot1=toplot1, Tk=Tk, ncompl=numcompletepep,
            grp=grp) # yuliya: added complete, removed nosib.miss.info
  return(retval)
}


# Second portion of EigenMS: remove effects of bias and rescale to avoid overfitting  
eig_norm2 <- function(rv) { #  keepnosib=0){
  ...
  # 
  # OUTPUT: normalized - matrix of normalized abundances with 2 columns of protein and peptdie names
  ...
  # if(n.u.treatment > 1) {
    ...
    # #mtmp <- model.matrix(~bl+it+Pi+tp, data=treatment, contrasts=list(bl="contr.sum", it="contr.sum",Pi="contr.sum", tp="contr.sum"))
  # } else {
    # #modt = try(as.numeric(t(ftemp)), silent=TRUE)  # do we really need a try here? just building model mat
    # mtmp = as.numeric(t(ftemp)) #### yuliya: CHECK THIS!!! is this always a data.frame? does it work the same here?
  # }
 
  treatment = data.frame(treatment) # does this work?
  ...
  for (i in 1:nrow(pres)) {
    if(i%%100 == 0) {
      print(paste('Processing peptide ',i)) 
    }
  ...
  if(!inherits(modt, "try-error")) { # do nothing if could not make model matrix
    ...
    if(!inherits(bhat, "try-error")) {
        ...
        bhat_n <-  solve(t(modt) %*% modt) %*% t(modt) %*% norm_m[i, pos]
        betahat_n[,i] <- bhat_n
        ceffects_n <- modt %*% bhat_n
        resm_n[pos] <- norm_m[i,pos] - ceffects
        newR[i, ] <- resm_n
	  } else {
	      print(paste('got exception 2 at peptide:', i, 'should not get here...')) 
	      exPeps[i] = 2 # should not get 2 here ever...
	  }
	} else {
	  print(paste('got exception at peptide:', i)) 
	  exPeps[i] = 1 # keep track of peptides that threw exeptions, check why...
	}
}
  
# ################## begin rescaling values ############################
   # maxrange to be sqrt(maxrange)
   ...
   if(max_e > 1 && max_e <= 2) {
     ...
     maxrange <- max_e # sqrt((round(max_e*10)/10)/2) 
   }
   #try with 0.5 and 1
 
# UNCOMMENT and implement?!
print("Now rescaling. This takes a bit longer...")
print(paste("Upper rescaling factor:",maxrange))


#####################################################################################
####  Cannot rescale if have only 1 treatment group, do not rescale
####  OR: randomly saparate into 2 grps, compute p-values, rescale... not implemented
nGrpFactors = n.u.treatment # length(colnames(treatment))
 
if(nGrpFactors == 1) {
  # check to see if have more than 1 treatment group
  nGrps = n.u.treatment 
}  else {
  nGrps = n.u.treatment # number of facotrs passed in, have multiple treaments
}
if(class(treatment) == "factor") { # TRUE if one factor
  if(n.u.treatment == 1) { 
    nGrps = n.u.treatment
  } else {
    nGrps = 1 # no rescaling possible, no p-values to ajust for... 
    # Could randomly subset into 2 groups and comute p-values, 
    # since it is random should be Uniform. COuld rescale to approximate Uniform distr.
  }
} else { # data.frame
  nGrps = n.u.treatment
}
   
#############################    NO rescaling in this version!!!!!! #####################
nGrps = 1  # setting this variable to 1 prevents going into the if portion
# else in line 667 sets y_rescaled to norm_m
# NOTE: need NOT to use solve, use lm() as above
if(nGrps > 1) {
  p_correct <- rep(NA, nrow(pres))
  p_rescaled <- rep(NA, nrow(pres))
  r_factor <- rep(NA, nrow(pres))
  y_rescaled <- matrix(NA, nrow(pres), ncol(pres)) 
  
 
  ftemp = treatment 
  options(warn = -1)
  lm.fm = makeLMFormula(ftemp, 'ftemp') # using general function that can accomodate for 1+ number of factors
  mod <- try(model.matrix(lm.fm$lm.formula, data=ftemp, eval(parse(text=lm.fm$lm.params))), silent=TRUE)
  # mod <- model.matrix(~bl+it+Pi+tp, data=ftemp, contrasts=list(bl="contr.sum", it="contr.sum",Pi="contr.sum", tp="contr.sum"))
  options(warn = 0)
  
  X_0 <- cbind(cbind(c(rep(1, dim(mod)[1]))), V0)  # used to be dim(tretment)
  X_1 <- cbind(mod, V0)
  
  
  for (tt in 1:nrow(pres)){
    # check if we got all NAs in norm_m, if so we could not normalize this peptide, skip it
    y1 <- as.numeric(pres[tt,])
    y1 <- as.vector(na.omit(y1)) 
    if(length(y1) > 0) {  # if not all NAs, means we were able to normalize, thus can rescale also
      if(tt %% 100 == 0) {
        print(paste("Processing peptide", tt))
      }
     
      y1.bar <- as.numeric(norm_m[tt,])    # this are normalized intensities
      y1.bar <- as.vector(na.omit(y1.bar))
      X_null <- X_0[!is.na(pres[tt,]),]   # X_0 has intercept and eigenpepts, eliminate those that were NAs in y1
      X_unrestricted <- X_1[!is.na(pres[tt,]),] # X_1 has interceptm, grp, eigenpepts,
      if(n.treatment == 1) { # 1 factor, whatever number of levels  was n.u.treatment
        ftemp <- data.frame(treatment[!is.na(pres[tt,]),])
        colnames(ftemp) = 'ftemp'
      } else {    
        ftemp <- data.frame(treatment[!is.na(pres[tt,]),]) # do not need to set colnames? come from data frame
        tmp1 = cbind(y1,ftemp)
        #colnames(ftemp) = colnames(treatment[!is.na(pres[tt,])])
      }
      # compute p-values for experimental factors of interest using a model that also includes
      # residual eigenpeptides computed by EigenMS
      # correct p-values
      
      options(warn = -1)
      # this is odd and does not work
      #fit1 <- try(lm(y1 ~ftemp, contrasts=list(ftemp="contr.sum")), silent=TRUE)
      
      # not used:s
      fit1_lm.fm = makeLMFormula(ftemp, 'ftemp') # using general function that can accomodate for 1+ number of factors
      #modt <- try(model.matrix(lm.fm$lm.formula, data=treatment, eval(parse(text=lm.fm$lm.params))), silent=TRUE)lm.fm = makeLMFormula(treatment) # using general function that can accomodate for 1+ number of factors
      #fit1_modt <- try(model.matrix(fit1_lm.fm$lm.formula, data=ftemp, eval(parse(text=fit1_lm.fm$lm.params))), silent=TRUE)
      #bhat <-  try(solve(t(fit1_modt) %*% fit1_modt) %*% t(fit1_modt) %*% y1)
      options(warn = 0)
      #### if(!inherits(bhat, "try-error")) {
      fit_null <- lm(y1~ X_null[, -1])
      fit_unrestricted <- lm(y1~ X_unrestricted[, -1])
      p_correct[tt] <- anova(fit_null, fit_unrestricted)[6][2,]
      
      # fit model with groups as factors only, get residuals...
      # fit2 <- lm(y1.bar ~ ftemp,  contrasts=list(ftemp="contr.sum"))
      
      
      fit2_lm.fm = makeLMFormula(ftemp, 'ftemp') # using general function that can accomodate for 1+ number of factors
      fit2_modt <- try(model.matrix(fit2_lm.fm$lm.formula, data=ftemp, eval(parse(text=fit2_lm.fm$lm.params))), silent=TRUE)
      
 
      # get betas from lm()
      # yuliya: use lm() to get residuals
      if(n.treatment == 1) {
        fit_forbetas <- lm(y1 ~ ftemp, data=ftemp)  # may need an if statement here for 1 factor vs multi factor
      } else {  
        fit_forbetas <- lm(y1 ~ fit2_modt[,-1], data=ftemp)
      }
      # bhat2 <- solve(t(fit2_modt) %*% fit2_modt) %*% t(fit2_modt) %*% y1.bar
      bhat2 <- coefficients(fit_forbetas)
      Ceffects = fit2_modt %*% bhat2
      ei = y1.bar - Ceffects
      
      # H.c <- mod.c %*% solve(t(mod.c) %*% mod.c) %*% t(mod.c)  # in R we can use lm()
      # if(!inherits(H.c, "try-error")) {
      # R.c <- Y.c - t(H.c %*% t(Y.c))  # R.c - matrix of residuals
      # betahat = inv(X' * X) * X' * m;
      # Ceffects = X*betahat;
      # resm = m - Ceffects;
      #}
      #ei <- resid(fit2)  # residuals, this may be rescaled
      # sd.fit1 <- sd(resid(fit1)) # not sure what this is for....
      
      # this is the range of rescaling values to try [0 maxrange]
      g.arr = seq(0, maxrange, by=0.1) # can be done outside of the loop...
      gp.arr <- rep(NA, length(g.arr))
      p.mod <- rep(NA, length(g.arr))
      y1.temp <- rep(NA, length(g.arr))
    } # end if not all NAs in norm_m, else we do nothing
    
    for(ii in 1:length(g.arr)){

      gr <- g.arr[ii]
      eir = ei + sign(ei)*gr
      y1.bar.mod <- y1.bar-ei + eir
      fit.mod <- lm(y1.bar.mod ~ ., data=ftemp)  # this is rescaled
      ss <- summary(fit.mod)$fstatistic
      p.mod[ii] <- pf(ss[1], ss[2], ss[3], lower=FALSE)
      gp.arr[ii] <- (abs(p.mod[ii] - p_correct[tt]))
    }
    
    # if something went wrong we should not do the following
    if(!any(is.finite(gp.arr))) next; 
    # gp.arr is the difference between correct and rescaled p-value, thus selecting min
    p_rescaled[tt] <- p.mod[which(gp.arr==min(gp.arr, na.rm=TRUE))]
    y_r <- y1.bar + sign(ei)*g.arr[which(gp.arr==min(gp.arr, na.rm=TRUE))]
    r_factor[tt] <-g.arr[which(gp.arr==min(gp.arr, na.rm=TRUE))]
    
    kk=1
    y_rescale <- array(NA, n.treatment)
    # yuliya: going through each element 1 by 1, can be made more efficient if keep track of present positons
    for (jj in 1:length(pres[tt,])){
      if (!is.na(pres[tt,jj])) {   # pres[t,] has more elements if there are NAs
        y_rescale[jj] <- y_r[kk]
        kk <- kk + 1
      } else {
        y_rescale[jj]=NA
    ...
    y_rescaled[tt,] <- y_rescale
  } # for loop tt
  
} else {
  y_rescaled = norm_m # for 1 group normalization only, we do not rescale
}
  # yuliya: add column names to y-rescaled, now X1, X2,...
  colnames(y_rescaled) = colnames(pres) # these have same number of cols

  # rownames(y_rescaled) <- rownames(rv$m)[as.numeric(rownames(pres))] #rownames(pres)
  ...
  final = y_resc
  # yuliya: row names are assumed to be UNIQUE, feature/component ID...
  ...
  # colnames(final) <-  colnames(rv$m) # can add column names... now done outside of EigenMS, rownames also
  ...
  return(list(normalized=final, norm_m=y_rescaled, eigentrends=V0, rescrange=maxrange, 
              norm.svd=toplot3, exPeps=exPeps, norm_no_resc=norm_m))
}
...
#    sv.sig: significance level for surrogate variables
  ...
  ndf = length(dstat) # sticking to Tom's variable name, guessing this is what it should be
print(paste('length(dstat) = ndf = ', ndf))
  dstat0 <- matrix(0,nrow=B,ncol=ndf) # num samples, I think

print("Starting Bootstrap.....")
# this is the Bootstrap procedure that determines the number of significant eigertrends... 
  for(ii in 1:B){
    if(ii %% 10 == 0) { print(paste('Iteration ', ii)) }
